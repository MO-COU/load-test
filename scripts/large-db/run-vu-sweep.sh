#!/usr/bin/env bash
# INSERT만 / INSERT+UPDATE 두 워크로드를 여러 VU 단계로 돌려 한 파일에
# 누적 기록하고, 끝에 요약 표를 붙인다. 총 요청 수는 VU가 달라져도
# ~10,000건으로 고정한다(VU * ITERATIONS_PER_VU ~= TOTAL_REQUESTS).
#
#   bash scripts/large-db/run-vu-sweep.sh <stage-label> [vu1 vu2 ...]
#
# 예: bash scripts/large-db/run-vu-sweep.sh stage0 10 30 50 100 200 500
#
# 결과: docs/benchmark-results/sync-vs-async/raw/<stage-label>.tsv
#       (원본 시계열 + 파일 끝에 요약 표)
#
# 같은 <stage-label>로 다시 실행하면 파일을 지우지 않고 이어쓴다.
# 이미 있는 (workload, vu) 조합을 다시 넘기면 그 조합의 기존 데이터·요약만
# 지우고 새로 기록한다(중복 방지).

set -uo pipefail

STAGE="${1:?사용법: run-vu-sweep.sh <stage-label> [vu1 vu2 ...]}"
shift
VU_LIST=("$@")
if [ ${#VU_LIST[@]} -eq 0 ]; then
  VU_LIST=(10 30 50 100 200 500)
fi
...
if [ -f "${OUT_FILE}" ]; then
  # 이어쓰기 모드: 기존 "# Summary" 블록을 떼어내 보존하고, 데이터 영역만 남긴다.
  # 주의: grep -E 의 '\t' 는 이 환경에서 탭으로 해석되지 않아 매칭이 항상
  # 실패했었다(요약 유실 버그). $'\t' 로 실제 탭 문자를 넣어야 한다.
  TAB="$(printf '\t')"
  ...
  # 이번에 다시 돌릴 (workload, vu) 조합은 기존 데이터·요약에서 미리 제거(중복 방지)
  ...
  printf 'workload\tvus\titerations_per_vu\ttotal_requests\telapsed_in_run_sec\thikari_active\thikari_pending\ttomcat_busy\ttomcat_max\tinnodb_buf_reads\tinnodb_buf_read_requests\tinnodb_buf_pages_free\tinnodb_buf_pages_total\tinnodb_os_log_fsyncs\tinnodb_row_lock_waits\n' \
    > "${OUT_FILE}"
fi
...
get_field() {
  # $1=innodb snapshot text, $2=variable name
...
run_one() {
  ...
  local max_active max_pending max_busy max_rowlock
  ...
  SUMMARY_ROWS+=("$(printf '%s\t%s\t%s\t%s\t%ss\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s(exit=%s)' \
    "$workload" "$vus" "$iters" "$total" "$elapsed" \
  ...
  echo "   -> elapsed=${elapsed}s max_active=${max_active:-NA} max_pending=${max_pending:-NA} max_busy=${max_busy:-NA} k6_exit=${k6_exit}"
}

for vus in "${VU_LIST[@]}"; do
  run_one "insert-only" "large-db-insert.js" "$vus"
done

for vus in "${VU_LIST[@]}"; do
  run_one "insert-update" "large-db-insert-then-update.js" "$vus"
done

{
  echo ""
  echo "# ===== Summary (Stage: ${STAGE}) ====="
  echo "# workload  vus  iterations_per_vu  total_requests  elapsed  max_hikari_active  max_hikari_pending  max_tomcat_busy  max_row_lock_waits  fsync_delta  buf_reads_delta  buf_read_requests_delta  k6_summary"
  {
