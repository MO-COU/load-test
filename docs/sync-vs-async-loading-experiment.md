...
1. actuator·micrometer 의존성을 추가해 HikariCP 지표를 노출한다.
2. 비동기(배치) 적재 경로를 구현한다.
3. 동기·비동기 각각 INSERT 10,000건, UPDATE+INSERT 각 10,000건을 측정한다.
4. 부하를 점진적으로 올리며 각 방식이 무너지는 지점(지연 급증·오류율 상승·풀
   고갈)을 기록한다.
5. 두 방식의 connection pool 사용량, 처리량, 정합성을 나란히 비교한다.
6. 결과를 바탕으로 ERD 개선안을 정리한다.
...
- 스키마: `scripts/large-db/schema.sql`, 시드: `scripts/large-db/seed-stage.sql`
  ...
  정확히 10,000건, 신규 추가)
- k6 스크립트: `load-test/large-db-insert.js`(INSERT만),
  `load-test/large-db-insert-then-update.js`(INSERT+UPDATE, 신규 추가)
  (`VUS`, `ITERATIONS_PER_VU` 환경변수로 요청 건수 조절 가능)
- 동기 적재 API: `BenchmarkCouponIssueController` / `Service` / `Repository`
  (`JdbcTemplate` 단건 처리)
- actuator·micrometer 의존성 및 `/actuator/prometheus` 노출 — 추가 완료
...
- 커넥션 풀·Tomcat 스레드·InnoDB 버퍼 풀 자동 기록 —
  ...
  `collect-pool-metrics.sh`(Git Bash/WSL) 둘 다 신규 추가

아래는 이 실험을 위해 새로 만들어야 한다.

- 비동기(배치) 적재 경로
- 부하를 점진적으로 올려 실패 지점을 찾는 k6 시나리오
- 결과 기록 위치: [`benchmark-results/sync-vs-async/`](benchmark-results/sync-vs-async/README.md)
